    ...
    public class SymmetricEncryption : Encryption
    {
        ...
        public override byte[] Encrypt(byte[] valueBytes, byte[] initializationVector = null)
        {
            ...
            using (var output = new MemoryStream())
            {
                ...
                using (
                    var cryptoOutput = new CryptoStream(output,
                        _algorithm.CreateEncryptor(_secretKey, initializationVector), CryptoStreamMode.Write))
                {
        ...
        public override byte[] Decrypt(byte[] encryptedValue, byte[] initializationVector = null)
        {
            ...
            using (var output = new MemoryStream())
            {
                using (
                    var cryptoOutput = new CryptoStream(output,
                        _algorithm.CreateDecryptor(_secretKey, initializationVector), CryptoStreamMode.Write))
                {
