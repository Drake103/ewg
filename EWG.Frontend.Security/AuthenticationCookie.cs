...
namespace EWG.Frontend.Security
{
    public class AuthenticationCookie
    {
        ...
        private AuthenticationCookie(byte[] data)
        {
            using (var memoryStream = new MemoryStream(data))
            {
                using (var binaryReader = new BinaryReader(memoryStream))
                {
                    ...
                    _email = binaryReader.ReadString();
        ...
        public AuthenticationCookie(short cookieType, Guid id, bool persistent, string email, string username, string[] roles = null, byte[] tag = null)
        {
